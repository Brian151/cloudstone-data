window.SINGLE_TAB = " ";
window.ImgCollapsed = "http://formatjson.com/images/collapse.gif";
window.ImgExpanded = "http://formatjson.com/images/expand.gif";
...
function Process() {
	...
	var json = $id("UnformattedJSON").value;
	var html = "";
	try {
		if (json == "")
			json = "\"\"";
		...
		$id("BeautifiedCode").innerHTML = "<PRE class='CodeContainer'>"+html+"</PRE>";
	} catch (e) {
		alert("The JSON is not well formated:\n" + e.message);
		$id("BeautifiedCode").innerHTML = "";
	}
}
...
function CollapseAllClicked() {
	EnsureIsPopulated();
	TraverseChildren($id("BeautifiedCode"), function(element) {
...
function CollapseLevel(level) {
	EnsureIsPopulated();
	TraverseChildren($id("BeautifiedCode"), function(element, depth) {
		if (element.className == 'collapsible') {
			if (depth >= level) {
				MakeContentVisible(element, false);
			} else {
...
function EnsureIsPopulated() {
	if (!$id("BeautifiedCode").innerHTML && !!$id("UnformattedJSON").value)
		Process();
}
...
function SelectAllClicked() {
	...
	var range = (!!document.body && !!document.body.createTextRange) ? document.body
			.createTextRange()
			: document.createRange();

	if (!!range.selectNode)
		range.selectNode($id("BeautifiedCode"));
	else if (range.moveToElementText)
		range.moveToElementText($id("BeautifiedCode"));

	if (!!range.select)
		range.select($id("BeautifiedCode"));
	else
